////////////////////////////////////////////////////////////////////////
// Class:       LongDoublePulseTree
// Plugin Type: analyzer (art v2_09_02)
// File:        LongDoublePulseTree_module.cc
//
...
class LongDoublePulseTree : public art::EDAnalyzer {
public:
  explicit LongDoublePulseTree(fhicl::ParameterSet const & p);
  // The compiler-generated destructor is fine for non-base
  // classes without bare pointers or other resource use.

  // Plugins should not be copied or assigned.
  LongDoublePulseTree(LongDoublePulseTree const &) = delete;
  LongDoublePulseTree(LongDoublePulseTree &&) = delete;
  LongDoublePulseTree & operator = (LongDoublePulseTree const &) = delete;
  LongDoublePulseTree & operator = (LongDoublePulseTree &&) = delete;
...
LongDoublePulseTree::LongDoublePulseTree(fhicl::ParameterSet const & p)
  :
  EDAnalyzer(p),
  fitterModuleLabel_(
...
void LongDoublePulseTree::beginRun(art::Run const &r)
{
...
void LongDoublePulseTree::analyze(art::Event const &e)
{
  fillGainPerRun(e);
  fillGainSummary(e);
}
...
void LongDoublePulseTree::endRun(art::Run const &r)
{
  closeGainPerRun();
  closeGainSummary();
}
...
void LongDoublePulseTree::endJob()
{
  // Summarize the data in a tree.
  art::ServiceHandle<art::TFileService> tfs;
  TTree *tree = tfs->make<TTree>("GainData", "Long Double Pulse Gain Data");
...
void LongDoublePulseTree::initGainPerRun()
{
...
void LongDoublePulseTree::fillGainPerRun(art::Event const &e)
{
  // Make sure we want the plots.
  if (!writeGainPerRun_) {
    return;
...
void LongDoublePulseTree::closeGainPerRun()
{
  if (!writeGainPerRun_) {
    return;
  }
...
void LongDoublePulseTree::initGainSummary()
{
...
void LongDoublePulseTree::fillGainSummary(art::Event const &e)
{
  if (!writeGainSummary_) {
    return;
  }
...
void LongDoublePulseTree::closeGainSummary()
{
  if (!writeGainSummary_) {
    return;
  }
...
DEFINE_ART_MODULE(LongDoublePulseTree)
