...
class PileupCorrector:
    ...
    def rhoDoubleIterable(self, binx, nBinsY):
            print(binx)
            #if(self.verbosity > 0):
            #    print("    ", binx,"/",nBinsY+1)
            rhos = []
    ...
    def ComputeRhoDouble(self):
        ...
        for binx in range(1,nBinsX+1):
            ...
            p.apply_async(self.rhoDoubleIterable, [binx, nBinsY], callback = self.log_result)
        p.close()
        p.join()
        for (binx, ycont) in self.result_list:
            for biny, rhoi in enumerate(ycont):
                self.rhoDouble.SetBinContent(binx,biny,rhoi)
